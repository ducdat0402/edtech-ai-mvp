...
class _AdminPanelScreenState extends State<AdminPanelScreen>
  ...
  Future<void> _loadAnalytics() async {
    ...
    try {
      final apiService = Provider.of<ApiService>(context, listen: false);
      final data =
          await apiService.getAnalyticsOverview(period: _analyticsPeriod);
      if (mounted) {
        setState(() {
          _analyticsData = data;
  ...
  Widget _buildAnalyticsTab() {
    ...
    final engagement =
        _analyticsData!['engagement'] as Map<String, dynamic>? ?? {};
  ...
  Widget _buildAnalyticsStatCard(String label, String value,
  ...
  Widget _buildSubjectCompletionChart(Map<String, dynamic> learning) {
    final data =
        List<Map<String, dynamic>>.from(learning['completionsBySubject'] ?? []);
        ...
        SizedBox(
            ...
            child: BarChart(BarChartData(
                    ...
                    sideTitles: SideTitles(
                        ...
                        getTitlesWidget: (value, meta) {
                          ...
                          if (idx >= 0 && idx < data.length) {
                            final name = '${data[idx]['subject']}'.length > 8
                                ? '${data[idx]['subject']}'.substring(0, 8)
                                : '${data[idx]['subject']}';
                            return Padding(
              ...
              barGroups: List.generate(data.length, (i) {
                final completed =
                    double.tryParse('${data[i]['completed']}') ?? 0;
  ...
  Widget _buildRevenueMetricsCards(Map<String, dynamic> revenue) {
    ...
    final formatted = totalRevenue >= 1000000
  ...
  Widget _buildTopContributorsTable(Map<String, dynamic> content) {
    final contributors =
        List<Map<String, dynamic>>.from(content['topContributors'] ?? []);
