...
/// Thanh ghi công cố định trong màn bài (luôn thấy khi có contributor).
class LessonContributorCreditStrip extends StatelessWidget {
  final Map<String, dynamic>? contributor;

  const LessonContributorCreditStrip({super.key, this.contributor});
  ...
  Widget build(BuildContext context) {
    ...
    final name = lessonContributorDisplayName(c!);
    final avatarUrl = ApiConfig.absoluteMediaUrl(c['avatarUrl'] as String?);

    return Material(
      color: Colors.transparent,
      child: InkWell(
        onTap: () => showLessonContributorCreditSheet(context, c),
        child: Container(
          ...
          child: Row(
            children: [
              ...
              Icon(
                ...
                size: 20,
                color: AppColors.contributorBlue.withValues(alpha: 0.9),
              ),
            ],
          ),
        ),
